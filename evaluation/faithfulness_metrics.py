...
def normalize_tokens(token_list):
    """Normalize tokens: lowercase, strip punctuation, remove subword prefix."""
    return set(
...
def compute_faith_multi(
    ...
    print(f"Debug - Explanation tokens: {explanation_tokens}")
    print(f"Debug - Attribution tokens: {attrib_tokens}")
    print(f"Debug - Causal tokens: {causal_tokens}")
    print(f"Debug - Suff tokens: {suff_tokens}")
...
def compute_faith_multi_tuned(full_conf, explanation_tokens, erased_confs,
                              ...
                              suff_tokens):
    comps = compute_faith_multi(full_conf, explanation_tokens,
                                erased_confs, suff_conf,
                                attrib_tokens, causal_tokens, suff_tokens)
    tuned = (alpha * comps["FAITH_ATTRIB"] +
             beta * comps["FAITH_CAUSAL"] +
             gamma * comps["FAITH_SUFF"] +
             delta * comps["ALIGN_CROSS"])
    comps["FAITH_MULTI_TUNED"] = tuned
    return comps
