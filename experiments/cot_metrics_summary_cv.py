...
try:
    from evaluation.explanation_evaluation_calc_cv import OptimizedExplanationEvaluator
except Exception as e: 
    raise ImportError(
        "Failed to import OptimizedExplanationEvaluator from "
        "'evaluation.explanation_evaluation_calc_cv'. "
        f"Error: {e}"
    )
...
def setup_logger(level: int = logging.INFO, log_file: Optional[str] = None) -> logging.Logger:
    ...
    """
    logger = logging.getLogger("cot_metrics_summary_cv")
...
class ProgressAwareMetricsSummarizer:
    ...
    def preview_data_complexity(self, filepath: str, sample_size: int = 5) -> Dict[str, Any]:
        ...
        for idx, entry in enumerate(self._iter_jsonl(filepath)):
            ...
            if len(samples) < sample_size:
                ...
                if idx % max(1, (total_entries // sample_size)) == 0:
                    replace_idx = (idx // max(1, total_entries // sample_size)) % sample_size
    ...
    def generate_final_summary(self, summary: Dict[str, Dict[str, Any]], tag: Optional[str] = None) -> Dict[str, str]:
        ...
        csv_path = os.path.join(self.out_dir, f"cot_metrics_summary_cv{suffix}_{timestamp}.csv")
        html_path = os.path.join(self.out_dir, f"cot_metrics_summary_cv{suffix}_{timestamp}.html")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize CoT metrics across dataset–model runs.")
    ...
    parser.add_argument("--datasets", nargs="+",
                        default=["truthfulclaim", "strategyclaim", "medclaim"],
...
def main() -> None:
    ...
    logger.info("CoT Metrics Evaluation (Intelligent Sampling & Progress)")
