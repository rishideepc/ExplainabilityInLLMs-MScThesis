...
class AttentionPatcher:
    """
    Installs CustomLlamaAttention for a temporary forward pass, then restores 
    ...
    def install(self, target_indices, last_n_layers=1):
        """
        Replaces last-N self attention modules with CustomLlamaAttention 
        ...
        print(f"[PATCH] Installing CustomLlamaAttention on last {last_n_layers} layer(s)")
        for i in range(total - last_n_layers, total):
            original_layer = layers[i].self_attn
            self._backup[i] = original_layer

            custom_attn = CustomLlamaAttention(self.model.config, i, target_indices)
            try:
                custom_attn.load_state_dict(original_layer.state_dict(), strict=False)
                print(f"   Layer {i}: loaded state dict")
            except Exception as e:
                print(f"   Layer {i}: load_state_dict error: {e}")
            layers[i].self_attn = custom_attn.to(device).half()
            print(f"   Layer {i}: swapped -> CustomLlamaAttention(target_indices={target_indices})")

    def restore(self):
        """
        Restores original self attention modules 
        
        @params: 
            none 
        
        @returns: 
            transforms model back to original attention layers
        """
        if not self._backup:
            return
        layers = self.model.model.layers
        for i, attn in self._backup.items():
            layers[i].self_attn = attn
            print(f"   Layer {i}: restored original self_attn")
        self._backup.clear()
        print("[PATCH] Restore complete")


def logits_label_on_last_token(tokenizer, logits):
    """
    Reads 'TRUE' or 'FALSE' from final prediction logits 
    
    @params: 
        tokenizer, logits tensor 
        
    @returns: final label, probability for 'TRUE', probability for 'FALSE'
    """
    ...
    p_true = probs[true_ids[0]].item() if true_ids else 0.0
    p_false = probs[false_ids[0]].item() if false_ids else 0.0
    label = "TRUE" if p_true >= p_false else "FALSE"
    return label, p_true, p_false


def run_with_attention_perturbation_on_prompt(
    model,
    tokenizer,
    prompt: str,
    target_indices,
    ...
    @params: model, tokenizer, runner prompt, target indices, number of last layers being perturbed
    ...
    patcher = AttentionPatcher(model)
    try:
        patcher.install(target_indices, last_n_layers=last_n_layers)
        with torch.no_grad():
            outputs = model(**inputs, output_attentions=True)
    finally:
        patcher.restore()

    label, p_true, p_false = logits_label_on_last_token(tokenizer, outputs.logits)
    return label, p_true, p_false
